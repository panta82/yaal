...
function safeSet(ob, val, key, duplicateKeyFormat) {
	var duplicateKeyIndex;
	if (duplicateKeyFormat === true) {
		duplicateKeyFormat = ob._yaal.hashDuplicateKeyFormat;
	}
	else if (duplicateKeyFormat > 0) {
		duplicateKeyIndex = duplicateKeyFormat;
		duplicateKeyFormat = ob._yaal.hashDuplicateKeyFormat;
	}
	return libTools.safeSet(ob._yaal.reserved, ob, key, val, duplicateKeyFormat, duplicateKeyIndex);
}
...
function YaalInternal(forOb, make, reserved) {
	Object.defineProperties(this, {
		...
		clone: {
			value: function clone(empty) {
				var cloned = make(forOb._yaal.type);
				libTools.shallowCopy(forOb, cloned, empty);
				libTools.shallowCopy(forOb._yaal, cloned._yaal, empty);
				return cloned;
			}
		},
		isEmptyMember: {
...
function yaalFlatten(index, fluid) {
	...
	var isHash = !libTools.isArray(ob),
		pushOrSet = getPushOrSet(isHash, ob._yaal.hashPropertyNumberingFormat),
		pick = false,
		searchDirection;
...
function yaalCompact() {
	var ob = this,
		pushOrSet = getPushOrSet(!libTools.isArray(ob), ob._yaal.hashPropertyNumberingFormat),
